...
2. **Churn model (Part 5)**  
   A behavioral LightGBM trained on about 200k wallets. It predicts `active_m3` (any trade between days 60 and 120 after first trade) using only the first seven days of behavior. SHAP explains the scores, and there is a Streamlit demo called **Churn Radar**.

Everything treats a taker wallet as a user. That works well at platform scale, but one person can have multiple wallets. Treat individual scores as triage signals, not identity.
...
**Ever-returned** is closer to what some sources mean when they cite ~75% return:
...
Writes `data/raw/churn_features.csv` (about 200k wallets, 12.5% sample).
...
**Held-out test metrics (behavioral model):** ROC-AUC ~0.67, PR-AUC ~0.53, top-decile churn lift ~1.5x. See `models/metrics_behavioral.json` for the full numbers.
...
- **Large CSV in git:** `churn_features.csv` is about 28 MB, under GitHub's 100 MB file limit.
...
- **active_m3:** any trade in days 60 to 120 (what the churn model predicts).
